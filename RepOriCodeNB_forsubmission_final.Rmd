...
Classic_oril=oril
save(bedlist, matlist, Classic_oril, file=paste(datapath,'Data_may19.Rdata', sep='/'))
...
```{r}
##'###############################################################Without efficiency
##Note: Assortativity is better calculated on continuous values, work is in progress to define better measures for binary variables (presence/absence of oris for example). However, we have observed that for practical purposes the results of looking at assortativity of origin presence are in line with those obtained looking at origin efficiency (a continuous variable). 
...
par(oma=c(1,1,1,1))
plot( colSums(featmat)[sel],unlist(OriAs), ylim=c(-0.04,0.08), pch=20,col=cols[sel],xlab='Number of nodes containing origins in PCHiC', ylab='OriAs in PCHiC network',
 cex.axis=1.5, cex.lab=1.5)
for (i in 1:100){
points( colSums(featmat)[sel],chasrandsimpldf[,i], col=cols[sel])
}
text( colSums(featmat)[sel],unlist(OriAs), pos=1,offset=-10, sel, cex=1.5, srt=90)
...
par(oma=c(1,1,1,1))
plot( colSums(featmat[which(rownames(featmat) %in% V(Gchicwtpromoe)$name),])[sel],unlist(OriAspoe), ylim=c(-0.2,0.2), pch=20,col=cols[sel],xlab='Number of nodes containing origins in PO subnetwork', ylab='OriAs in PO subnetwork',cex.axis=1.5, cex.lab=1.5)
for (i in 1:100){
points( colSums(featmat[which(rownames(featmat) %in% V(Gchicwtpromoe)$name),])[sel],chasoriAsrandprespromoeldf[,i], col=cols[sel])
}
text( colSums(featmat[which(rownames(featmat) %in% V(Gchicwtpromoe)$name),])[sel],unlist(OriAspoe), pos=1,offset=-8, sel, cex=1.5, srt=90)
...
elGori=get.edgelist(Gdistori)
rownames(elGori)=paste(elGori[,1], elGori[,2], sep='-')
E(Gdistori)$enames=rownames(elGori)
...
plot(density(log10(distintertadori[!is.na(distintertadori)])), col='darkgreen',lwd=3,  xlab='Distance spanned (log10 bases)',  ylim=c(0,1), cex=1.5, cex.lab=1.5, cex.axis=1.5, main='')
...
legend(6.5,1 ,lty=1, lwd=3,col=c('black', 'cyan', 'darkgreen'), legend=c('All', 'Intra-TAD', 'Inter-TAD'), bty="n", cex=1.5)
dev.off()
...
Part II Correlations of efficiency, degree

Correlation degree and efficiency


For RT correlations this part see RepOriCodeRT_forsubmission.Rmd
...
pdf(paste(datapath,'Figures/Supp/Efficiency_deg_3datasets.pdf', sep='/'), width=16, height=10)
par(tcl=0.4)
par(oma=c(3,3,3,0))
par(mar=c(8,8,8,3))
par(mfrow=c(1,3))
...
if (ds=='WT'){
boxplot(ef~deg, varwidth=T,  border=cols[ds], names=c(0:lim,lim2),  outline=F, xlab='Degree', ylab='Origin efficiency',ylim=c(2,20),
 main=paste(ds,', cor=',round(c$est,2),star), cex.axis=2.5, cex.lab=2.5, cex.main=2.5, tcl=0)
}
if (ds!='WT'){
  boxplot(ef~deg, varwidth=T,  border=cols[ds], names=c(0:lim,lim2),  outline=F, xlab='Degree', ylab='',ylim=c(2,20),
 main=paste(ds,', cor=',round(c$est,2),star), cex.axis=2.5, cex.lab=2.5, cex=2.5, cex.main=2.5, tcl=0)
}
}
dev.off()
...
pdf(paste(datapath,'Figures/Supp/Efficiency_vs_Degree_allori.pdf', sep='/'))
...
chasonlyoril=lapply(Goril, function(x){
  return(calc_assort(x, featefmat))
})
chas=calc_assort(Goril[['WT']], featefmat)
  plot( mean(featefmat[which(rownames(featefmat) %in% V(Goril[['WT']])$name),'WT']),chas[['WT']], pch=20,col=cols[sel],ylim=c(-0.1,0.5),
main='WT')
for (s in 1 :length(sel)){
  print(sel[s])
  print(mean(featefmat[which(rownames(featefmat) %in% V(Goril[[sel[s]]])$name),sel[s]]))
  chas=calc_assort(Goril[[s]], featefmat)
  points( mean(featefmat[which(rownames(featefmat) %in% V(Goril[[sel[s]]])$name),sel[s]]),chas[sel[s]], pch=20,col=cols[sel],
main=sel)
  text(mean(featefmat[which(rownames(featefmat) %in% V(Goril[[sel[s]]])$name),sel[s]]),chas[sel[s]], label=sel[s])
}
```
