...
Read ori efficiencies centred on original ori centers
...
```
Add origin feature at each resolution
...
#save(wtC25_5kb_randlnew29july, file=paste0('wtC25_5kb_randlnew',nrand,'_dataset.Rdata'))
...
```
Calculate chas for randomized networks
```{r}
...
```{r}
###  compartments (from Bonev et al. 2017)
comps=read.table(paste0(datapath,'/cav/BonevCompartments.csv'), sep=',', header=T)

compsbin=comps
compsbin[,4]=(compsbin[,4]=='A') 
compsbin[,4]=sapply(compsbin[,4], as.numeric) 

###now load compartment info as a feature

wtnetefl25COMP=wtnetefl25

 wtnetefl25COMP[[ "wt_scores_5kb.df"]]<- load_features(wtnetefl25COMP[[ "wt_scores_5kb.df"]], compsbin, type='features_table',missingv=0, auxfun=mean, featnames='comp')
 
  wtnetefl25COMP[[ "wt_scores_25kb.df"]]<- load_features(wtnetefl25COMP[[ "wt_scores_25kb.df"]], compsbin, type='features_table',missingv=0, auxfun=mean, featnames='comp')
  
   wtnetefl25COMP[[ "wt_scores_10kb.df"]]<- load_features(wtnetefl25COMP[[ "wt_scores_10kb.df"]], compsbin, type='features_table',missingv=0, auxfun=mean, featnames='comp')

   
   ###write files with edges 
#save(wtnetefl25COMP[[ "wt_scores_5kb.df"]],
     #,  wtnetefl25COMP[[ "wt_scores_25kb.df"]], wtnetefl25COMP[[ "wt_scores_10kb.df"]], 
 #    file=paste0(datapath,'/VOCHiC_networks_forKaustav.RData'))
   
   
   ###identify which nodes are A or B
   
  Acompnodes5= wtnetefl25COMP[[ "wt_scores_5kb.df"]]$nodes[which(wtnetefl25COMP[[ "wt_scores_5kb.df"]]$features[,'Compartment']==1)]
  
  Bcompnodes5= wtnetefl25COMP[[ "wt_scores_5kb.df"]]$nodes[which(wtnetefl25COMP[[ "wt_scores_5kb.df"]]$features[,'Compartment']<1)]
  
Acompnodes10= wtnetefl25COMP[[ "wt_scores_10kb.df"]]$nodes[which(wtnetefl25COMP[[ "wt_scores_10kb.df"]]$features[,'Compartment']==1)]
  
  Bcompnodes10= wtnetefl25COMP[[ "wt_scores_10kb.df"]]$nodes[which(wtnetefl25COMP[[ "wt_scores_10kb.df"]]$features[,'Compartment']<1)]

  Acompnodes25= wtnetefl25COMP[[ "wt_scores_25kb.df"]]$nodes[which(wtnetefl25COMP[[ "wt_scores_25kb.df"]]$features[,'Compartment']==1)]
  
  Bcompnodes25= wtnetefl25COMP[[ "wt_scores_25kb.df"]]$nodes[which(wtnetefl25COMP[[ "wt_scores_25kb.df"]]$features[,'Compartment']<1)]

  
  
##create A subnetwork for each compartment
  
  wtnetefl25COMPA_5kb=subset_chromnet(wtnetefl25COMP[[ "wt_scores_5kb.df"]], method="nodes", nodes1=Acompnodes5)
  
  wtnetefl25COMPB_5kb=subset_chromnet(wtnetefl25COMP[[ "wt_scores_5kb.df"]], method="nodes", nodes1=Bcompnodes5)
  
  COMPA5kb_chas=chas(wtnetefl25COMPA_5kb)
  COMPA5kb_feat=export(wtnetefl25COMPA_5kb)
  
  COMPB5kb_chas=chas(wtnetefl25COMPB_5kb)
  COMPB5kb_feat=export(wtnetefl25COMPB_5kb)
  
  
  
  wtnetefl25COMPA_10kb=subset_chromnet(wtnetefl25COMP[[ "wt_scores_10kb.df"]], method="nodes", nodes1=Acompnodes10)
  
  wtnetefl25COMPB_10kb=subset_chromnet(wtnetefl25COMP[[ "wt_scores_10kb.df"]], method="nodes", nodes1=Bcompnodes10)

  wtnetefl25COMPA_25kb=subset_chromnet(wtnetefl25COMP[[ "wt_scores_25kb.df"]], method="nodes", nodes1=Acompnodes25)
  
  wtnetefl25COMPB_25kb=subset_chromnet(wtnetefl25COMP[[ "wt_scores_25kb.df"]], method="nodes", nodes1=Bcompnodes25)

  
```

```{r}
  ###randomization within each compartment 
  nrand=50
    #wtnetefl25COMPA_5kb_rand50=randomize(wtnetefl25COMPA_5kb,nrandom=nrand, preserve.nodes = NULL, dist.match=T)
    
    #wtnetefl25COMPB_5kb_rand50=randomize(wtnetefl25COMPB_5kb,nrandom=nrand, preserve.nodes = NULL, dist.match=T)
 
 #save(wtnetefl25COMPA_5kb_rand50, wtnetefl25COMPB_5kb_rand50, file=paste0(datapath, '/Compartmentsrand50.RData'))
  load(paste0(datapath, '/Compartmentsrand50.RData'))  
    
COMPA5kb_randlchas=lapply( wtnetefl25COMPA_5kb_rand50, chas)
COMPA5kb_randlfeat=lapply (wtnetefl25COMPA_5kb_rand50, export)


COMPB5kb_randlchas=lapply( wtnetefl25COMPB_5kb_rand50, chas)
COMPB5kb_randlfeat=lapply (wtnetefl25COMPB_5kb_rand50, export)

```
Plotting OriEfAs in specific A or B compartments
```{r}
pdf(paste0(datapath,'/CompartmentAnalysisOriEfAs50rand.pdf'))

#plotchas(wtfeatl25[[3]], wtchasl25[[3]], wtC25_5kb_randlfeat,wtC25_5kb_randlchas ,xmin=0.25, xmax=2.5, ymin=-0.1, ymax=0.3, labon='ON', title='VOCHiC 5kb', exclude='allOris|exclusive|ALL-ORI|r1|r2$|r3|r4|r5|r6|r7|r8|r9|r10', 1)
#dev.off()


#pdf(paste0(datapath,'/CompartmentAnalysisOriEfAs50randA.pdf'))
plotchas(COMPA5kb_feat, COMPA5kb_chas, COMPA5kb_randlfeat,COMPA5kb_randlchas ,xmin=0.25, xmax=2.5, ymin=-0.1, ymax=0.3, labon='ON', title='VOCHiC COMPA 5kb', exclude='allOris|exclusive|ALL-ORI|r1|r2$|r3|r4|r5|r6|r7|r8|r9|r10|Compartment', 1)
#dev.off()

#pdf(paste0(datapath,'/CompartmentAnalysisOriEfAs50randB.pdf'))
plotchas(COMPB5kb_feat, COMPB5kb_chas, COMPB5kb_randlfeat,COMPB5kb_randlchas ,xmin=0.25, xmax=2.5, ymin=-0.1, ymax=0.3, labon='ON', title='VOCHiC COMPB 5kb', exclude='allOris|exclusive|ALL-ORI|r1|r2$|r3|r4|r5|r6|r7|r8|r9|r10|Compartment', 1)
#dev.off()
```
...
```
Function for distance analysis
```{r}
#Distance analysis
distne<-function(ne){
...
Annotation of fragments and oris with respect to active promoters, TSS etc.
```{r}
...
pdf(paste0(datapath,'/Effvsdeg_nonP.pdf'))
boxplot(degnonP5~quantilesEffsnonP5, varwidth=T, xlab='Efficiency quartiles', ylab='Degree', cex.axis=1, cex.lab=1.5, ylim=c(0,400))
dev.off()
cor.test(effsnonP5,degnonP5)
...
pdf(paste0(datapath,'/Effsvsdeg_all.pdf'))  
boxplot(deg~quantilesEffs, varwidth=T, xlab='Efficiency quartiles', ylab='Degree', cex.axis=1, cex.lab=1.5, ylim=c(1,400))
dev.off()
...
#save(wtC25_5kb_randlRT, file=paste0(datapath, '/RTFeb2021/wtC25_5kb_randlRT.Rdata'))
...
drRTchasdf=as.data.frame(drRTchas)[2,]
drRTchasmean=rowMeans(drRTchasdf)
drRTchassd=apply(drRTchasdf,1, sd)


zsP=0
for (d in 1:length(drRTchas)){
zsP[d]=round((drRTchasdf[d]-drRTchasmean)/drRTchassd,2)
}
names(zsP)=names(drRTchas)
...
Load TAD coordinates from Bonev et al. 2017 and map VOCHiC fragments to TADs
```{r}
...
RTdiffcompA[i]=abs(sample(RTvals[Acompnodes5],1)-sample(RTvals[Acompnodes5],1))
RTdiffcompB[i]=abs(sample(RTvals[Bcompnodes5],1)-sample(RTvals[Bcompnodes5],1))
...
#pdf(paste0(datapath,'/RTdif_VOCHiC_comp.pdf'), height=5, width=11)  
intrae=setdiff(names(RTdif), intere)
boxplot(RTdif[intere], RTdif[setdiff(names(RTdif), intere)],RTdif,RTdiffrand,RTdiffcompA, RTdiffcompB, outline=F, names=c('Inter-TAD', 'Intra-TAD', 'All inter-origin', 'Random', 'Random A', 'Random B'), ylab='RT difference')


#to generate effect sizes and pvals
sublist=list(RTdif[intere], RTdif[intrae],RTdif,RTdiffrand,RTdiffcompA, RTdiffcompB)

nam=c('Inter-TAD', 'Intra-TAD', 'All inter-origin', 'Random', 'Random A', 'Random B')
bp <- boxplot(sublist, outline = F,names=nam, ylab='RT difference', ylim=c(0,8))
combs=list(c(1,2), c(2,3), c(1,3), c(3,4), c(3,5), c(3,6))
ycoord=4
for (c in combs ){

  signif <- t.test(sublist[[c[1]]],sublist[[c[2]]])

  if(signif$p.val<0.0001){
    xcoord0=c(1:length(sublist))[c[1]]
    xcoord1=c(1:length(sublist))[c[2]]

    arrows(x0 = xcoord0, x1=xcoord1 ,
       y0 = ycoord, y1=ycoord,
       angle = 90, length = 0.05, code = 3)
    text(x=(xcoord0+xcoord1)/2, y=ycoord, paste0('p=10^-',round(log10(signif$p.val),0)), pos=3)
    text(x=(xcoord0+xcoord1)/2, y=ycoord, round((signif$estimate[1]-signif$estimate[2]),3), pos=1)
  }
  ycoord=ycoord+0.5
}

text(1,5, 'try')
text(x = 1.5, y = max_val + 0.1*(max_val - min_val), labels = wt_vs_aph, pos = 3, cex = 0.7)


text(x = (1:length(sublist))[signif],
y = bp$stats[5,][signif], labels = "*", pos = 3, cex = 2,  xpd = T)
...
##perform randomization (since TAD networks are much smaller this can be run easily)
nrand=50
...
#pdf('TADOriEfAs.pdf')
plot(rep(mean(oritadfeat),length(oritadchas)), unlist(oritadchas), ylim=c(0, 0.3), xlab='Efficiency averaged across TAD network nodes', ylab='TADOriEfAs', pch=21, col='red', bg='red')
